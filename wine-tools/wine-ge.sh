...
tr() {
    case "$LANG_UI:$1" in
        en:INFO) echo "\nInfo: versions above 8.15 seem not to work on Batocera.";;
        fr:INFO) echo "\nInfo : les version superieur à 8.15 semblent ne pas fonctionner sous batocera.";;
        en:FETCH) echo "\nFetching Wine GE-Custom versions...";;
        fr:FETCH) echo "\nRécupération des versions de Wine GE-Custom...";;
        ...
        en:MENU_TITLE) echo "GE-Custom";;
        fr:MENU_TITLE) echo "GE-Custom";;
...
#info
(
  dialog --backtitle "Foclabroc Toolbox" --title "Wine-GE-Custom" --msgbox "$(tr INFO)" 8 60
  sleep 1
) 2>&1 >/dev/tty
...
# Récupérer la version depuis le fichier temporaire pour l'utiliser plus tard
	...
	if [[ $? -ne 0 ]]; then
		(
            dialog --backtitle "Foclabroc Toolbox" --infobox "$(tr DL_CANCEL "$version")" 6 60
			sleep 1
		) 2>&1 >/dev/tty
		continue
	fi
