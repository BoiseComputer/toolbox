...
EOF
    ...
    TXT_MENU_RECORD_TITLE="Recording Menu"
    ...
    TXT_RECORDING_SAVED="\nVideo capture saved successfully in the Recordings folder.\n"
    ...
    TXT_TOOLS_SCREENSHOT="[Screenshot] -> Take Batocera screenshots."
    TXT_TOOLS_RELOAD="[Reload]     -> Refresh the game list."
    TXT_TOOLS_RECORD="[Record]     -> Capture Batocera screen videos"
    TXT_TOOLS_INFOS="[Infos]      -> Show Batocera system information."
    TXT_TOOLS_BACK="[Back]       -> Return to the main toolbox menu"
    TXT_SCREENSHOT_SAVED="\nScreenshot saved successfully in the Screenshots folder."
    ...
    MENU_MAIN_13="[Tools]               -> Batocera tools. Screenshots, records..."
    MENU_MAIN_14="[Install in Ports]    -> Add this menu to Batocera ports"
    MENU_MAIN_15="[Underground]         -> !!!Password required !!!"
...
EOF
    ...
    TXT_MENU_RECORD_TITLE="Menu Enregistrement"
    ...
    TXT_TOOLS_SCREENSHOT="[Screenshot] -> Prendre des captures d'écran de Batocera."
    TXT_TOOLS_RELOAD="[Reload]     -> Actualiser la liste des jeux."
    TXT_TOOLS_RECORD="[Record]     -> Capturer des vidéos de l'écran de Batocera"
    ...
    MENU_MAIN_13="[Tools]               -> Outils pour Batocera. Screenshot, Records..."
    MENU_MAIN_14="[Install dans Ports]  -> Ajoute ce menu aux ports Batocera"
    MENU_MAIN_15="[Underground]         -> !!!Mot de passe nécessaire !!!"
...
show_intro() {
  ...
  dialog --backtitle "Foclabroc Toolbox" --ok-label "$TXT_INFO_OK_LABEL" --title "$TXT_WELCOME_TITLE" --textbox "$tmpfile2" 20 78 &
    pid=$!
    sleep 3
    kill "$pid"
}
...
tools_options() {
  ...
  start_recording_menu() {
    CHOICE=$(dialog --title "$TXT_MENU_RECORD_TITLE" --backtitle "Foclabroc Toolbox" --menu "$TXT_MENU_RECORD_PROMPT" 15 60 4 \
      1 "$TXT_RECORD_MANUAL" \
      2 "$TXT_RECORD_15" \
      3 "$TXT_RECORD_35" \
      4 "$TXT_BACK" \
    ...
    while true; do
      ...
      case $CHOICE in
        1)
          # Option Screenshot
          batocera-screenshot
          show_message "$TXT_SCREENSHOT_SAVED"
          ;;
        2)
          # Option Reload
          curl http://127.0.0.1:1234/reloadgames
          show_message "$TXT_RELOAD_DONE"
          ;;
        3)
          # Option Record
          start_recording_menu
...
main_menu() {
    while true; do
        ...
        --menu "$TXT_MAIN_MENU_PROMPT" 24 90 16 \
        1 "$MENU_MAIN_1" \
        2 "$MENU_MAIN_2" \
        3 "$MENU_MAIN_3" \
        4 "$MENU_MAIN_4" \
        ...
        case $main_menu in
            ...
            3)
                ...
                clear
                curl -Ls https://raw.githubusercontent.com/foclabroc/toolbox/refs/heads/main/fpinball/fpinball.sh | bash
                ;;
            4)
